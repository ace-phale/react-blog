...
const PostDetails = (props) => {
  ...
  const displayTags = tags.map((tag) => (
    <Link className='tag mr-2 pr-2' key={tag} style={{ textDecoration: 'inherit' }} to={'/tag/' + tag}>
      #{tag}
    </Link>
  ));
  ...
  return (
    <>
      <Container className='px-0' style={{ minWidth: '100%' }}>
        <Container className='px-0 pt-md-3 mb-3' style={{ minWidth: '100%', position: 'relative', paddingBottom: '56.2%' }}>
